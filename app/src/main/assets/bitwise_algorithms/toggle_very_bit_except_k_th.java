 ...
 * Input  :  n = 1, k = 1
 * Output : 4294967292
 * 4294967262 has all bits toggled except second bit from right.
 **/
public class Solution {
    public int toggleEveryBitExceptKth(int n, int k) {
        return n ^ (~(1 << (k-1)));
    }

    public int toggleEveryBitExceptKth2(int n, int k) {
        return ~(n ^ (1 << (k-1)));
    }
}